...
class LibHeaderGenerator(LibGenerator):
    ...
    def generate(self):
        ...
        self.doc.scopeEnter()
        self.doc.line("STROMX_{0}_API int stromxRegister{1}"
...
class LibImplGenerator(LibGenerator):
    ...
    def generate(self):
        ...
        self.doc.line("int stromxRegister{0}(stromx::runtime::Registry* registry)"\
        ...
        self.doc.scopeExit()
        self.doc.line("catch(stromx::runtime::Exception & e)")
        self.doc.scopeEnter()
        self.doc.line("std::cerr << e.what() << std::endl;")
        self.doc.line("return 1;")
        self.doc.scopeExit()
        
        self.doc.blank()
        self.doc.line("return 0;")
...
class CMakeGenerator(LibGenerator):
    ...
    def generate(self):
        ...
        self.doc.line("${stromx_RUNTIME_LIB}")
        self.doc.line("${stromx_CVSUPPORT_LIB}")
        self.doc.decreaseIndent()
        self.doc.line(")")
        self.doc.blank()
...
class ConfigGenerator(LibGenerator):
    ...
    def generate(self):
        ...
        self.doc.line('#define STROMX_{0}_LOCALE_DOMAIN "libstromx_{1}"'\
        ...
        self.doc.increaseIndent()
        self.doc.line("#define STROMX_{0}_HELPER_DLL_IMPORT __declspec(dllimport)".format(p))
        self.doc.line("#define STROMX_{0}_HELPER_DLL_EXPORT __declspec(dllexport)".format(p))
        self.doc.decreaseIndent()
        self.doc.line("#else")
        self.doc.increaseIndent()
        self.doc.line("#define STROMX_{0}_HELPER_DLL_IMPORT".format(p))
        self.doc.line('#define STROMX_{0}_HELPER_DLL_EXPORT __attribute__ ((visibility ("default")))'.format(p))
        self.doc.decreaseIndent()
        self.doc.line("#endif // WIN32")
        self.doc.blank() 
        ...
        self.doc.increaseIndent()
        self.doc.line("#define STROMX_{0}_API STROMX_{0}_HELPER_DLL_EXPORT".format(p))
        self.doc.decreaseIndent()
        self.doc.line("#else // stromx_{0}_EXPORTS".format(self.p.ident))
        self.doc.increaseIndent()
        self.doc.line("#define STROMX_{0}_API STROMX_{0}_HELPER_DLL_IMPORT".format(p))
        self.doc.decreaseIndent()
