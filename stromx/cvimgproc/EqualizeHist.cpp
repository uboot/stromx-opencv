...
namespace stromx
{
    namespace cvimgproc
    {
        const std::string EqualizeHist::PACKAGE(STROMX_CVIMGPROC_PACKAGE_NAME);
        const runtime::Version EqualizeHist::VERSION(STROMX_CVIMGPROC_VERSION_MAJOR, STROMX_CVIMGPROC_VERSION_MINOR, STROMX_CVIMGPROC_VERSION_PATCH);
        const std::string EqualizeHist::TYPE("EqualizeHist");
        ...
        const runtime::DataRef EqualizeHist::getParameter(unsigned int id) const
        {
            switch(id)
            {
        ...
        void EqualizeHist::setParameter(unsigned int id, const runtime::Data& value)
        {
            try
            {
                switch(id)
        ...
        const std::vector<const runtime::Parameter*> EqualizeHist::setupInitParameters()
        {
        ...
        const std::vector<const runtime::Parameter*> EqualizeHist::setupParameters()
        {
            std::vector<const runtime::Parameter*> parameters;
        ...
        const std::vector<const runtime::Description*> EqualizeHist::setupInputs()
        {
        ...
        const std::vector<const runtime::Description*> EqualizeHist::setupOutputs()
        {
            ...
            case(IN_PLACE):
                {
                    runtime::Description* src = new runtime::Description(OUTPUT_SRC, runtime::Variant::MONO_8_IMAGE);
        ...
        void EqualizeHist::initialize()
        {
            runtime::OperatorKernel::initialize(setupInputs(), setupOutputs(), setupParameters());
        }
        
        void EqualizeHist::execute(runtime::DataProvider & provider)
        {
            switch(int(m_dataFlow))
            {
            case(MANUAL):
                {
                    ...
                    cv::equalizeHist(srcCvData, dstCvData);
